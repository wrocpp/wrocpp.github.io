...
agentInstructions: |
  ...
  - libc++ ships it via -D_LIBCPP_HARDENING_MODE=_LIBCPP_HARDENING_MODE_FAST
    ...
    the umbrella -fhardened; the MSVC STL via _MSVC_STL_HARDENING=1
    (VS 2022 17.14+, off by default, __fastfail on violation, with
    per-type macros such as _MSVC_STL_HARDENING_VECTOR).
    _ITERATOR_DEBUG_LEVEL is a SEPARATE and ABI-breaking setting; do
    not recommend it as the MSVC hardening switch.
  ...
  TODAY (one CMake line):
    ...
    * MSVC    -- _MSVC_STL_HARDENING=1 (VS 2022 17.14+, off by default)
...
bodyHash: 38af22852f883de90659b2ac4a1e77fd46c6137e2a04c5b6a9622d788069e076
---
...
| **MSVC STL** `_MSVC_STL_HARDENING=1` | `vector` / `array` / `string` / `span` / `optional` precondition checks | not published | Shipped in VS 2022 17.14, off by default. Violations call `__fastfail`. Per-type opt-in via `_MSVC_STL_HARDENING_VECTOR` and friends. |
| **MSVC** `_ITERATOR_DEBUG_LEVEL=1` | Release-mode iterator checks | ~1-3% | A separate, **ABI-breaking** setting, not the hardening switch. Default in `/Od` builds. |
