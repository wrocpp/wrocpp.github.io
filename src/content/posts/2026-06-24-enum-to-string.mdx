...
The inverse — string back to flags — is the same shape with `std::views::split` (C++20) doing the tokenisation, no manual `find('|')` loop:
...
constexpr std::optional<E> from_flags_string(std::string_view s) {
    ...
    for (auto part : std::views::split(s, '|')) {
        ...
        if (!match) return std::nullopt;     // typo at boundary -> reject
...
This is the trust-boundary version: a typo in a config file or CLI flag returns `std::nullopt` rather than a silently-zero mask or a partial match. Same `from_string` lookup as the single-value path, OR-combined under the underlying type.
...
<GodboltEmbed id="qEjo8ME17" title="Goodbye magic_enum -- reflection-driven enum to/from string + flags round-trip (GCC 16.1)" />
