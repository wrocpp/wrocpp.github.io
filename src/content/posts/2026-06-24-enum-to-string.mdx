...
The inverse — string back to flags — is a ranges chain: split on `|`, monadic `.and_then` / `.transform` over `std::optional<U>` propagating the fail-closed step, `std::ranges::fold_left` (C++23) doing the OR-combine, final `.transform` putting the value back into the enum type:
...
The trust-boundary semantics fall out of the monad: any token that fails to look up returns `nullopt` from the inner `from_string`, the inner `.transform` short-circuits to `nullopt`, the outer `.and_then` propagates it, and every subsequent `acc.and_then(...)` call is a no-op. A typo in a config file or CLI flag returns `std::nullopt` rather than a silently-zero mask or a partial match.
...
<GodboltEmbed id="vYsfvz3Yh" title="Goodbye magic_enum -- reflection-driven enum to/from string + flags round-trip (GCC 16.1)" />
