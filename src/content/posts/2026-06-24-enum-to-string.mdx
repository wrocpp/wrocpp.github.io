...
The inverse — string back to flags — is the same shape with one extra step (split on `|`):
...
constexpr std::optional<E> from_flags_string(std::string_view s) {
    ...
    while (true) {
        ...
        if (!match) return std::nullopt;     // typo at the boundary -> reject
...
<GodboltEmbed id="MTr1q8P6h" title="Goodbye magic_enum -- reflection-driven enum to/from string + flags round-trip (GCC 16.1)" />
