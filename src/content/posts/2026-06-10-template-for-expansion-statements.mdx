...
Reflection gives you a list of members. Splicing puts a single member back into code. What connects them is the *expansion statement* from [P1306](https://wg21.link/P1306):
...
This is the seed of [post 5](/posts/enum-to-string/): enum-name/value mappings without `magic_enum`'s compiler tricks.
...
<GodboltEmbed id="TODOtplfor" title="template for — unroll over members" />

## What's next

Arc 1 is done. Arc 2 builds real tools out of these three primitives:

- **[Post 5 — Goodbye `magic_enum`](/posts/enum-to-string/)** — enum ↔ string without compiler tricks.
- **[Post 6 — Auto `std::formatter<T>`](/posts/auto-formatter/)** — pretty-printing any aggregate.
- **[Post 7 — Derive equality, hash, ordering](/posts/derive-eq-hash/)**.
