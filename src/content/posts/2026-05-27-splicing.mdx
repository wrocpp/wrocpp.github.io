...
[Post 2](/posts/first-reflection/) covered reflecting *into* `std::meta::info`. The hard question was: how do we get *out* again — how do we use a reflection as if we'd written the original name?
...
<GodboltEmbed id="TODOsplicing" title="Splicing — reflections back to code" />
...
- **[Post 4 — `template for`](/posts/template-for-expansion-statements/)** — loops that unroll at compile time, the natural partner to splicing.
- **[Post 8 — A 40-line JSON serializer](/posts/json-naive/)** — full payoff: `^^` + `[:r:]` + `template for` fused into a working library.
