...
> **Could the demo self-toggle?** Conceptually, yes -- you would wrap pattern (3) in something like `if constexpr (! consteval-test on ^^fibs)` and the body would silently disappear when `fibs` is consteval, leaving patterns (1) and (2) to run. The trick is that the matching reflection predicate -- proposed as `std::meta::is_consteval` in [P3795R0 "Miscellaneous Reflection Cleanup"](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2025/p3795r0.html) -- is **not yet shipped in clang-p2996**, and no reliable pre-P3795 workaround discriminates the two specifiers in this compiler today. (Tried: a `requires` expression around the call, and one around an address-take. Both return `true` for both kinds; the language-level constraint fires later than the `requires` body.) For now, the honest reproducer is to flip the keyword on line one *and* comment out pattern (3) together. When P3795 lands, this demo gets a one-line upgrade -- and reflection has officially started introspecting the qualifiers of the very functions that produced its values.
