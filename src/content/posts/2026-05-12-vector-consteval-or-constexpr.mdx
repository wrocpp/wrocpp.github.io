...
> **Why `consteval` here, when `constexpr` would also work?** Try it: change `consteval auto fibs(int n)` to `constexpr auto fibs(int n)` and the program still compiles and prints the same eight Fibonacci numbers. The reason it works is that `define_static_array` is itself `consteval`, so the whole expression `define_static_array(fibs(8))` is evaluated in an *immediate-function context* -- and that context forces `fibs(8)` to run at compile time regardless of whether `fibs` is marked `constexpr` or `consteval`. The choice is about *intent and surface area*: a `constexpr` function is callable from runtime code too (the compiler must emit a runtime symbol), while a `consteval` function is a hard guarantee that no runtime call site exists. I used `consteval` here because "this never runs at runtime" is the honest contract for a Fibonacci factory whose result we always pipe straight into `define_static_array`. If you also wanted to call `fibs(n)` at runtime with a runtime `n`, mark it `constexpr` -- the demo above still works.
