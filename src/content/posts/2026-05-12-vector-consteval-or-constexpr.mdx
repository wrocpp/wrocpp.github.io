...
```cpp
// fibs is `constexpr` (not `consteval`) -- see (3) below for why.
...
int main() {
    ...
    // (3) runtime call: real heap-allocated vector
    auto f3 = fibs(8);
...
<GodboltEmbed id="5qTnzc11h" gcc_id="9rec7xzE9" title="Three ways to consume a constant-time std::vector" />
