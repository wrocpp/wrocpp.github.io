...
Nested aggregates work automatically. `Address` has the same partial specialisation applied. Containers of aggregates work too: `std::vector<User>` prints via `std::formatter<std::vector<T>>` which recurses into each element's `std::formatter<User>`.
...
## The same walker, reused for equality and JSON
