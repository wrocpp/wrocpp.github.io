...
draft: false
discussion: https://github.com/wrocpp/wrocpp.github.io/discussions/50
---
...
1. **Marker annotations** are empty types used for their *presence*. For `rjson::skip{}`, you check `annotation_of_type<skip>(m).has_value()`.
...
- `user_id` → `"id"` via field-level `json_name` (overrides container policy, since explicit beats implicit).
...
| Go encoding/json | `json:"id"` | `json:"-"` | `json:",omitempty"` | none (tag per field) |
...
<GodboltEmbed id="f7hcnMx9o" title="serde-parity annotations in C++26" />

## The next two posts in the series
