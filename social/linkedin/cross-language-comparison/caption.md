...
## Body
Eighteen posts into the wro.cpp reflection arc. This one zooms out: five canonical reflection tasks (JSON serialize, enum to string, CLI parse, ORM row binding, mock generation) lined up across **seven languages**: Rust (serde), C# (System.Text.Json), Java (Jackson), TypeScript (decorators), Go (struct tags), Python (Pydantic), and **C++26**. Not to declare a winner. To be precise about what kind of reflection each gives you, at what cost, with what ceiling.
